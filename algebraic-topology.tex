...
the \textbf{\textit{$n$-th homotopy group}} are homotopy classes of maps
...
\begin{definition}
	A \textbf{\textit{weak factorization system $(\mathcal{L},\Rc)$}} on a category $\Mc$ is comprised of two clases of morphisms $\Lc$ and $\Rc$ so that
	\begin{enumerate}
		\item Every morphism in $\Mc$ may be factored as a morphism in $\mathcal{L}$ followed by a morphism in $\Rc$:
		$$\begin{tikzcd}
			\bullet\arrow[rr,"f"]\arrow[rd,"\mathcal{L}\ni\ell",swap]&&\bullet\\
			&\bullet\arrow[ru,"r\in\Rc",swap]
		\end{tikzcd}$$
		\item The maps in $\mathcal{L}$ have the \textbf{\textit{left lifting property}} with respect to each map in $\Rc$ and equivalently the maps in $\Rc$ have the \textbf{\textit{right lifting property}} with respect to each map in $\Lc$, that is, any commutative square
		$$\begin{tikzcd}
		...
		\item The classes $\mathcal{L}$ and $\Rc$ are each closed under retracts in the arrow category: given a commutative diagram
		$$\begin{tikzcd}
...
Verify that the class of morphisms $\mathcal{L}$ characterized by the left lifting property against a fixed class of morphisms $\Rc$ is closed under coproducts, closed under retracts, and contains the isomorphisms.
\end{exercise}

\subsubsection{Model category structure on a category}

\begin{definition}[Lecture]
	A \textbf{\textit{model structure}} on a category $\mathcal{A}$ is a choice of subcategories $\Wc,\mathcal{C},\Fc$ called \textbf{\textit{weak-equivalences}}, \textbf{\textit{cofibrations}} and \textbf{\textit{fibrations}} with the following properties:
	\begin{enumerate}
		...
		$(\mathcal{B},\Dc)$ is a weak factorization system. That is,
		\begin{enumerate}
			\item Any morphism in $\mathcal{A}$ can be factored as a morphism in $\mathcal{B}$ followed by a morphism in $\Dc$.
			\item Lifts:
			$$\begin{tikzcd}[scale cd=1.2]
				\bullet\arrow["f",d,swap]\arrow[r]&\bullet\arrow[d,"g"]\\
				\bullet\arrow[ur,dashed,"\exists"]\arrow[r]&\bullet
			\end{tikzcd}$$
			\item[(c')] Notice that the axiom of retracts is not necessary. $r'\in\Rc\iff$ it satisfies (b) for all $\ell\in\mathcal{L}$.
 		\end{enumerate}
	\end{enumerate}
\end{definition}
...
topological analog of a cokernel is the \textit{\textbf{cofiber}} $B/A$ of a
...
The inclusions $X\hookrightarrow Y_{n}\hookrightarrow Y_{n-1}\hookrightarrow
\ldots\hookrightarrow Y_{1}=K(\pi_{1}(X),1)$ are converted to fibrations, and in fact the fiber of $Y_{k}\to Y_{k-1}$ is a $K(\pi_{k}(X),k)$-space.
...
\begin{lemma}[Yoneda, \href{https://en.wikipedia.org/wiki/Yoneda_lemma}{wiki}]
	Let $F$ be a functor from a locally small category $\mathcal{C}$ to $\Set$. Then for each object $X$ of $\mathcal{C}$, the natural transformations $\mathbb{N}at(\Hom(X,-),F)$ are in one-to-one correspondence with the elements of $F(X)$, that is
	$$\mathbb{N}at(\Hom(X,-), F)\cong F(X)$$
...
\end{lemma}
\begin{lemma}
	$\mathbb{N}at (\Hom(-,X),\Hom(-,Y))\cong\text{Hom}(X,Y)$.
\end{lemma}
\begin{remark}
	The correspondence $X\mapsto\Hom(-,X)$ is fully faithful, that is, the correspondence $\Hom(X,X')\to\mathbb{N}at(\text{Hom}(-,X),\text{Hom}(-,X'))$ is injective and bijective.
\end{remark}
...
\end{tikzcd}$$
	\item If you put all the absolute groups $H_n(X_p)$ in a big $A=\bigoplus_{n,p\in \mathbb{N}}H_n(X_p)$ and all the relative $H_n(X_p,X_{p-1})$ in a big $E=\bigoplus_{n,p\in \N}H_n(X_p,X_{p-1})$, you can write all those long exact sequences in a compact triangle diagram called an \textit{\textbf{exact couple}} triangle diagram
	$$
	\begin{tikzcd}
